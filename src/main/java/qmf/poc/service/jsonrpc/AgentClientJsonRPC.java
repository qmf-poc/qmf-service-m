...
public class AgentClientJsonRPC implements AgentClient {
    ...
    @Override
    public CompletionStage<String> ping(String agentId, Object payload) {
        ...
        final CompletionStage<Object> future = transport.sendRequest(agentId, request);
        return future.thenApply(result -> (result instanceof String) ? (String) result : result.toString());
    }

    @Override
    public CompletionStage<RunResult> run(String agentId, String owner, String name, int limit) {
        ...
        return future.thenApply(result -> {
            if (result instanceof JsonObject jsonObject) {
                return new RunResult(jsonObject.getString("body"), owner, name);
            } else {
                return new RunResult("", owner, name);
            }
        });
    }

    @Override
    public CompletionStage<List<QMFObjectCatalog>> getCatalog(String agentId) {
